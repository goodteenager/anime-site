...
export async function generateMetadata({ params }: PageProps) {
  ...
  try {
    ...
    return {
      title: `${anime.title} — HanaAnime`,
      description: anime.description?.slice(0, 160),
    };
  } catch {
    return { title: "Аниме — HanaAnime" };
  }
}
    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div className="text-center">
          <h1 className="mb-2 text-2xl font-bold text-text-primary">Аниме не найдено</h1>
          <p className="mb-4 text-text-muted">Тайтл не найден или произошла ошибка.</p>
          <Link href="/catalog" className="text-accent font-semibold hover:underline">Вернуться в каталог</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="flex flex-col gap-8 lg:flex-row">
        <div className="shrink-0">
          <div className="relative mx-auto w-64 overflow-hidden rounded-2xl border border-border shadow-lg shadow-accent-light/10 lg:mx-0">
            <Image
          ...
          <div className="mt-4 flex flex-col gap-2">
            <Link
              ...
              className="pastel-btn justify-center"
            >
              <Play size={16} />
              Смотреть
            </Link>
        ...
        <div className="flex-1">
          <h1 className="mb-1 text-3xl font-bold text-text-primary">{anime.title}</h1>
          ...
          <div className="mb-6 flex flex-wrap items-center gap-3">
            ...
            {anime.min_age && (
              <span className="rounded-full bg-bg-hover px-3 py-1.5 text-sm text-text-secondary">
          ...
          {anime.genres && anime.genres.length > 0 && (
            <div className="mb-6">
              <h3 className="mb-2 text-sm font-bold text-text-muted">Жанры</h3>
              <div className="flex flex-wrap gap-2">
                {anime.genres.map((g) => (
                  <Link
                    key={g.value}
                    href={`/catalog?genres=${g.href}`}
                    className="rounded-full border border-border px-3 py-1 text-xs font-semibold text-text-secondary transition-colors hover:border-accent hover:bg-accent/10 hover:text-accent"
                  >
                    {g.title}
                  </Link>
                ))}
          ...
          {anime.studios && anime.studios.length > 0 && (
            <div className="mb-6">
              <h3 className="mb-2 flex items-center gap-1.5 text-sm font-bold text-text-muted">
                <Users size={14} />
                Студия
              </h3>
              <div className="flex flex-wrap gap-2">
          ...
          {anime.description && (
            <div className="mb-6">
              <h3 className="mb-2 text-sm font-bold text-text-muted">Описание</h3>
